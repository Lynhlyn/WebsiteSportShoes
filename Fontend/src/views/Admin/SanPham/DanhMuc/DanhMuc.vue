      ...
      <div class="card-body">
        ...
        <div class="table-responsive">
          <table class="table table-striped table-hover">
                ...
                <th>
                  <div class="d-flex align-items-center gap-2">
                    ...
                    <button 
                      ...
                      @click="toggleSort('tenDanhMuc')"
                    >
                      <i class="bi" :class="getSortIcon('tenDanhMuc')"></i>
                    </button>
                  </div>
                </th>
                <th>
            ...
            <tbody>
              ...
              <tr v-else-if="paginatedDanhMucs.length === 0">
                <td colspan="6" class="text-center">Không có dữ liệu</td>
              </tr>
              <tr v-for="(danhMuc, index) in paginatedDanhMucs" :key="danhMuc.id">
                <td>{{ startIndex + index + 1 }}</td>
                <td>{{ danhMuc.tenDanhMuc }}</td>
                <td>{{ formatDate(danhMuc.ngayTao) }}</td>
                <td>{{ formatDate(danhMuc.ngaySua) }}</td>
                <td>
                  <span 
                    class="badge"
                    :class="danhMuc.trangThai ? 'bg-success' : 'bg-danger'"
                  >
                    {{ danhMuc.trangThai ? 'Hoạt động' : 'Không hoạt động' }}
                  </span>
                </td>
                <td>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
          <div class="text-muted">
            Hiển thị {{ startIndex + 1 }} đến {{ endIndex }} trên tổng số {{ filteredDanhMucs.length }} mục
          </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ isEditing ? 'Cập nhật' : 'Thêm' }} danh mục</h5>
          ...
          <div class="modal-body">
            <form @submit.prevent="handleSubmit">
              <div class="mb-3">
                ...
                />
                <div class="invalid-feedback">{{ errors.tenDanhMuc }}</div>
              </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" ref="deleteModal">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Bạn có chắc chắn muốn xóa danh mục "{{ selectedDanhMuc?.tenDanhMuc }}"?</p>
...
const filteredDanhMucs = computed(() => {
  let result = [...danhMucs.value];
  
  if (searchQuery.value) {
    const query = searchQuery.value.toLowerCase();
    result = result.filter(dm => 
      dm.tenDanhMuc.toLowerCase().includes(query)
    );
  }
  
  result.sort((a, b) => {
    const aValue = a[sortField.value as keyof DanhMuc];
    const bValue = b[sortField.value as keyof DanhMuc];
...
const endIndex = computed(() => Math.min(startIndex.value + pageSize.value, filteredDanhMucs.value.length));

const paginatedDanhMucs = computed(() => {
  return filteredDanhMucs.value.slice(startIndex.value, endIndex.value);
});
  ...
  if (!formData.value.tenDanhMuc.trim()) {
   ...
   // Hiển thị hộp thoại xác nhận trước khi thực hiện thao tác thêm hoặc cập nhật
    const isConfirmed = window.confirm(
      isEditing.value ? 'Bạn có chắc chắn muốn cập nhật danh mục này không?' : 'Bạn có chắc chắn muốn thêm danh mục này không?'
    );
...
const showFormModal = (danhMuc?: DanhMuc) => {
  if (danhMuc) {
    formData.value = { ...danhMuc };
...
const showDeleteConfirm = (danhMuc: DanhMuc) => {
  selectedDanhMuc.value = danhMuc;
  bsDeleteModal?.show();
};
...
const confirmDelete = async () => {
  if (!selectedDanhMuc.value?.id) return;

  loading.value = true;
  try {
    await axios.delete(`${API_URL}/${selectedDanhMuc.value.id}`);
    await fetchDanhMucs();
    hideDeleteConfirm();
